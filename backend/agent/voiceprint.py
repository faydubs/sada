...
# ── وضع البروتوتايب (افتراضي مُفعّل): يجعل البصمة الصوتية تُخرج نتائج إيجابية
#    دائماً دون الحاجة لـ SpeechBrain أو فكّ صيغ صوت المتصفح (webm). هذا مناسب
#    للعرض/التجربة. لإيقافه واستخدام التحقق الحقيقي: VOICEPRINT_DEMO=0 في البيئة.
VOICEPRINT_DEMO = os.getenv("VOICEPRINT_DEMO", "1").lower() not in ("0", "false", "no")

# بُعد بصمة ECAPA = 192. في وضع البروتوتايب نولّد بصمة ثابتة (متجه وحدة) حتى
# ينجح التسجيل دائماً وتتطابق دائماً مع نفسها.
_DEMO_DIM = 192


def _demo_embedding() -> np.ndarray:
    """بصمة عرض ثابتة (متجه وحدة) — تطابق نفسها بنتيجة ~1.0."""
    v = np.ones(_DEMO_DIM, dtype=np.float32)
    return v / np.linalg.norm(v)


def _demo_positive_result() -> dict:
    """نتيجة تحقق إيجابية افتراضية لوضع البروتوتايب."""
    score = 0.93
    return {
        "is_match": True,
        "best_score": score,
        "label": "registered_auctioneer",
        "scores": {"full": score, "last_10s": score},
...
def create_voiceprint(audio_path: str) -> Optional[np.ndarray]:
    ...
    """
    # وضع البروتوتايب: أرجِع بصمة عرض ثابتة دائماً (تسجيل ناجح بلا اعتماد على
    # SpeechBrain أو فكّ صيغة webm). هذا ما يجعل شاشة "سجّل بصمتك" تنجح دائماً.
    if VOICEPRINT_DEMO:
...
def verify_voiceprint(
    ...
    """
    # وضع البروتوتايب: تحقّق إيجابي دائماً (الصوت "يطابق" الدلّال المسجّل).
    if VOICEPRINT_DEMO:
        return _demo_positive_result()
